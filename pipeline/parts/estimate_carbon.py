...
def estimate_carbon(patches, trees, gps_error):
    ...
    for site in gps_error.keys():
        ...
        y_range, x_range = np.mgrid[0:max_x_tree, 0:max_y_tree]
        pos = np.dstack((y_range, x_range))
        rv = multivariate_normal([max_x_tree/2, max_y_tree/2], [[gps_error[site][0], 0], [0, gps_error[site][1]]])
        gaussian = rv.pdf(pos)
...
if __name__ == "__main__":
    ...
    with rasterio.open(f"../../data/reforestree/wwf_ecuador/RGB Orthomosaics/{site}.tif") as raster:
        img = raster.read()
        img_shape = pad(img, 400).shape
    carbon = compute_carbon_distribution(site, img_shape, trees, gps_error)
